...
function generateocptmpResources()
{
  echo "--- generateocptmpResources"
...
function flattenFilesPath()
{
  ...
  do
    ...
    OUTFILE=${OUTTMP//\//-}
    cp $INFILE tmp/ocp/$OUTFILE
    # echo "Generated ocp resource: tmp/ocp/${OUTFILE}"
  done
  echo "Type ls tmp/ocp to check all generated resources."
}
...
function sanitizeTemplateVariables()
{
  ...
  sed s/'k8sNamespace'/'{{ k8sNamespace }}'/g -i tmp/ocp/*
...
function filesToVersionControl()
{
  ...
  mkdir -p ../../ocp/d-reloader

  cp -v tmp/ocp/*.yaml         ../../ocp/d-reloader/.
  tree
  rm -rf tmp
